...
/* Relative import so the route can be exercised under
   `node --test --experimental-strip-types`, which does not resolve the
   `@/*` bundler alias. */
import { HOST_SERVICES_PRESENT } from '../../lib/machine-surfaces.ts';

/**
 * Write endpoints, with the per-IP limiter each route actually applies.
 * The rows flagged `hostSupplied` describe routes that come with the
 * retrieval/summarisation service and the mailing list the host
 * supplies; on a deployment without them the rows are omitted rather
 * than describing endpoints that are not there.
 */
const WRITE_ENDPOINTS: ReadonlyArray<{
  path: string;
  limiter: string;
  hostSupplied?: true;
}> = [
  {
    ...
    limiter:
      'no limiter in the route; answers are precomputed where possible, and the route returns 503 when the upstream model is not configured',
    hostSupplied: true,
  },
  { path: '/api/csp-report', limiter: 'yes, route-local' },
  { path: '/api/ask', limiter: 'yes, shared assistant limiter', hostSupplied: true },
  { path: '/api/ask/summarise', limiter: 'no limiter in the route', hostSupplied: true },
  { path: '/api/notify', limiter: 'yes, 5 per 10 minutes', hostSupplied: true },
];

const WRITE_ENDPOINT_ROWS = WRITE_ENDPOINTS.filter(
  (e) => !e.hostSupplied || HOST_SERVICES_PRESENT,
)
  .map((e) => `| \`POST ${e.path}\` | ${e.limiter} |`)
  .join('\n');
