...
/* Invariant for everything rendered below: an absolute URL appears once
   ...
   repeated URL as a duplicate submission, so the canonical links live in
   the `## Links` section at the end and the fact block near the top only
   names them. Answer bodies from data/ask-seed.json are the remaining
   exception - they carry inline links written for the standalone
   /q/<slug> pages. */
...
- **Status**: Beta (solo-maintained)
- **License**: Apache 2.0
- **Language**: Python 3.12+
- **Package Manager**: uv / pip / pipx
- **Author**: Alex Chernysh
- **Links**: website, repository, package, docs, issue tracker - see the Links section at the end of this document
...
### From source
\`\`\`bash
git clone https://github.com/sipyourdrink-ltd/bernstein.git
cd bernstein
uv sync
\`\`\`
...
| OpenHands | OpenHands (formerly OpenDevin) agent | Any OpenAI-compatible |
| Open Interpreter | Open Interpreter runtime | Any OpenAI-compatible |
...
| Kimchi | Kimchi coding agent | Various |
...
| Mistral | Mistral Vibe CLI | Mistral models |
| Muse Code | Muse Code CLI agent | Various |
| Plandex | Plandex terminal agent | Any OpenAI-compatible |
| Junie | JetBrains Junie agent | JetBrains-backed |
| gptme | gptme terminal agent | Any OpenAI-compatible |
| Letta Code | Letta Code agent | Letta models |
| Devin terminal | Cognition Devin terminal runtime | Cognition models |
| Amazon Q Developer | Amazon Q Developer CLI | AWS models |
| CLM | CLM coding agent | Various |
| agy | agy CLI agent | Various |
| aichat | aichat all-in-one LLM CLI | Any OpenAI-compatible |
| Pydantic AI | Pydantic AI runtime (factory-built profile) | Any via Pydantic AI |
| Antigravity | Google Antigravity agent (Gemini-backed) | Gemini models |
| Python runtime | In-process Python task runtime | n/a |
| Computer use | Reference computer-use adapter | Anthropic |
...
Zero-config demo run (\`bernstein quickstart\` is a deprecated alias, removed in 4.0.0).

### \`bernstein init [--wizard]\`
Initialize a project (\`.sdd/\` + \`bernstein.yaml\`); \`--wizard\` runs the interactive setup (\`bernstein init-wizard\` is a deprecated alias, removed in 4.0.0).

### Verification & replay

- \`bernstein replay RUN_ID [--verify | --from-step N]\` - replay a past run step-by-step from its journal; \`--verify\` recomputes the journal's Merkle head and reports the first divergent step, \`--from-step N\` rebuilds deterministic state to step N.
- \`bernstein fork --run ID --from-step N\` - rewind a run to journal step N and branch a new run from its content-addressed worktree snapshot.
- \`bernstein audit verify\` / \`bernstein audit diagnose RUN_ID --signal S\` - verify audit-log integrity; \`diagnose\` names the first faulty step of a run and writes a signed diagnosis receipt that \`audit diagnose verify\` re-derives offline.
- \`bernstein lineage verify RUN_ID\` - recompute a run's full Merkle hash chain and every HMAC tag in its lineage spine.
- \`bernstein a2a verify --receipt FILE --response FILE\` - verify an inbound A2A response against its lineage receipt, offline.
- \`bernstein a2a publish --endpoint URL\` - emit agent-registry records advertising this node's signed capability card.

### Analysis & evaluation

- \`bernstein plan compile SPEC\` - compile a requirements document into a gated task graph with requirement-hash lineage.
- \`bernstein impact api|deps|blast\` - change-impact analysis: breaking-API changes, affected call sites, blast radius (\`dep-impact\` and \`blast-radius\` are deprecated aliases, removed in 4.0.0).
- \`bernstein eval\` - evaluation pipelines: golden suite, SWE-bench, ProgramBench; \`--reliability K\` emits a signed pass^k reliability receipt (\`bernstein benchmark\` is a deprecated alias, removed in 4.0.0).
- \`bernstein report postmortem|incident|commits\` - failed-task postmortems, incident reports, per-run git diff stats (\`postmortem\`, \`incident\`, \`commit-stats\` are deprecated aliases, removed in 4.0.0).
...
- \`bernstein acp serve\` - native Agent Client Protocol bridge; Zed and other ACP editors can dispatch tasks and stream agent output without leaving the editor.
- \`bernstein autofix\` - daemon that watches Bernstein-opened PRs, reads CI failure logs, spawns an agent against the failing worktree, and pushes a fix commit.
- \`bernstein connect <provider>\` - OS keychain-backed credential vault; enter GitHub, Linear, Jira, Slack, or Telegram credentials once, every subsequent run reads from the keychain.
- \`bernstein preview start\` - spin up the project's dev server inside the active worktree and expose it over a public HTTPS tunnel for review or webhook testing.
...
Bernstein can run as an MCP server (\`bernstein mcp\`, stdio or HTTP transport), exposing orchestration capabilities as tools. Tools are advertised in three cumulative tiers selected with \`--mcp-tier {core|standard|all}\` (default \`standard\`):
...
Repo-root \`plugin.json\` and \`mcp.json\` manifests (Agent Plugins v1.0.0 schema) let plugin-aware clients discover and launch the server.
...
/* The comparison section is supplied by the host as `data/comparison.md`
   and is spliced in here when present. A checkout without that file
   renders every other section unchanged. */
async function comparisonSection(): Promise<string> {
  try {
    const fs = await import('node:fs/promises');
    const path = await import('node:path');
    const raw = await fs.readFile(
      path.resolve(process.cwd(), 'data', 'comparison.md'),
      'utf8',
    );
    return `${raw.trim()}\n\n---\n\n`;
  } catch {
...
export async function GET() {
  const [posts, seedItems] = await Promise.all([
    getAllPosts(),
    getAllSeedItems(),
  ]);
  ...
  content = content.replace(
    '## Cloud Execution (Cloudflare)',
    (await comparisonSection()) + '## Cloud Execution (Cloudflare)',
  );

  /* Canonical Q&A block.
   *
   * The full text of every /q/<slug> answer, in deterministic order,
   * appended to the deep-dump. This is the content an LLM crawler
   * actually slurps when it pulls /llms-full.txt - the rest of the
   * file is overview prose, but the Q&A block is the citable surface.
   * Each entry exposes:
   *   - the question shape (matches /ask chip text)
   *   - the canonical url where the answer lives standalone
   *   - the answer body, unmodified from data/ask-seed.json
   *   - tags + related links so a citer can see the answer's
   *     neighbourhood without re-fetching the JSON
   *
   * Keep this list in sync with /ask and /q automatically - one
   * source (data/ask-seed.json) drives all three surfaces. */
  if (seedItems.length > 0) {
    const qaBlock = seedItems
      .map((item) => {
        const tags = item.tags.length ? `tags: ${item.tags.join(', ')}` : '';
        /* Related entries are site-relative paths, not absolute URLs.
           Every /q/<slug> target already carries its own absolute `URL:`
           line further down this same document, so spelling the cross
           links out in full re-emitted 93 URLs across 44 entries and made
           this file the largest source of repeated URLs on the discovery
           surfaces; the few non-q targets (/vs, /cli-quickstart) resolve
           against the site root and are listed in /llms.txt. The
           neighbourhood information is unchanged, only its form. */
        const related = item.related.length
          ? `related: ${item.related
              .map((r) => (r.startsWith('/') ? r : `/q/${r}`))
              .join(', ')}`
          : '';
        const meta = [tags, related].filter(Boolean).join('  |  ');
        return [
          `### ${item.q}`,
          ``,
          `URL: ${SITE_URL}/q/${item.slug}`,
          meta ? meta : null,
          ``,
          item.a,
        ]
    ...
    content += `\n\n---\n\n## Canonical Q&A\n\nShort, citable answers to the questions ai overviews and llm citers ask about Bernstein. Each entry also lives at \`${SITE_URL}/q/<slug>\` as a stand-alone page with QAPage JSON-LD.\n\n${qaBlock}`;
  }
