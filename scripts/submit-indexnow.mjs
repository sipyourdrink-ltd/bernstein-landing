 ...
 *
 * The IndexNow key file MUST already be served at `/{key}.txt` in
 * `public/`. bernstein.run currently serves two:
 *   - public/8ea034eabcf341b28ce27fc1ca510018.txt
 *   - public/96e234d7d788437b13b030b25652d885.txt
 * (both pre-existing from Bing Webmaster verification). This script
 * picks the first one alphabetically as the canonical key.
 */
...
/* If the future W2-A multi-engine module lands and ships a runtime
   config (e.g. via valkey://shared-cache), prefer that. Today: hard-
   coded list above. Read the cache only if explicitly enabled by
   `INDEXNOW_USE_CACHE=1`. */
