 ...
 * segments starting with a character it reserves (`app/api/%5Fsignals`
 * serves `/api/_signals`), so a future move of `.well-known` into an
...
test('every endpoint in the shared list has a catalog member', () => {
  ...
  for (const endpoint of MACHINE_ENDPOINTS) {
    assert.ok(
      ...
      `${endpoint.path} is in MACHINE_ENDPOINTS but absent from the catalog`,
    );
  }
});
...
test('the resolver is not vacuously passing', () => {
  ...
  assert.equal(resolve('/api/_signals/log'), path.join('app', 'api', '%5Fsignals', 'log', 'route.ts'));
