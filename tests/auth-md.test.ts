...
test('the per-IP limiter column matches the routes it names', () => {
  /* The table is rendered from WRITE_ENDPOINTS in the route, filtered
     by whether the host-supplied services are configured. So the test
     reads the rows the document actually emitted rather than keeping a
     second copy of the list: whatever /auth.md claims, it has to be
     true of the route on this checkout. */
  const routes = apiRoutes();
  const LIMITER_CALL = /RateLimiter\.check\(|Limiter\.check\(|rateLimiter\.check\(/;

  const rows = [...BODY.matchAll(/^\| `POST (\/api\/[^`]+)` \| (.+?) \|$/gm)].map(
    (m) => ({ url: m[1], claim: m[2] }),
  );

  assert.ok(
    rows.length > 0,
    'the write-endpoint table rendered no rows; WRITE_ENDPOINTS or the table markup changed',
  );
  assert.ok(
    rows.some((r) => r.url === '/api/csp-report'),
    '/api/csp-report is served unconditionally and must always appear in the table',
  );

  for (const { url, claim } of rows) {
    const src = routes.get(url);
    assert.ok(src, `${url} is described in /auth.md but no route file exists for it`);
    const claimed = claim.startsWith('yes');
    const actual = LIMITER_CALL.test(src);
    assert.equal(
      actual,
      claimed,
      `/auth.md says ${url} ${claimed ? 'has' : 'has no'} a per-IP limiter, but the ` +
        `route ${actual ? 'calls' : 'does not call'} one. Update WRITE_ENDPOINTS in ` +
