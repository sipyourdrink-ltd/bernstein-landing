 ...
 * every path the projections emit against the repo's routes and public
 * files. An entry whose route does not exist fails the suite. Nothing
 * goes in here because it would be nice to offer.
 *
 * Some endpoints depend on a retrieval and summarisation service that
 * whoever runs the host supplies; they are flagged `hostSupplied` and
 * pass through `servedEndpoints()`, which drops them on a deployment
 * where that service is unconfigured. A catalogue that named them
 * anyway would be advertising a permanent 503.
 *
...
export interface MachineEndpoint {
  ...
  /**
   * Backed by the retrieval and summarisation service, which is
   * supplied by whoever runs the host rather than by this repository.
   * An entry flagged here is advertised only when that service is
   * configured - see `servedEndpoints()`.
   */
  hostSupplied?: true;
}
...
/**
 * Whether the retrieval and summarisation service is wired up on this
 * deployment. The endpoints that call it answer only when one of these
 * is set; with neither, they would 503 on every request, so the
 * discovery documents must not offer them.
 */
export const HOST_SERVICES_PRESENT = Boolean(
  process.env.AI_GATEWAY_URL || process.env.OPENROUTER_API_KEY,
);

/**
 * The endpoints this deployment actually serves: everything unflagged,
 * plus the host-supplied ones when the service behind them is
 * configured. Both projections and `allDeclaredPaths()` read this
 * rather than the raw list, so a document can never advertise a path
 * this host will answer with 404 or a permanent 503.
 */
export function servedEndpoints(): readonly MachineEndpoint[] {
  return MACHINE_ENDPOINTS.filter((e) => !e.hostSupplied || HOST_SERVICES_PRESENT);
}

/** The endpoints a caller can usefully invoke. */
export function agentCapabilities(): readonly MachineEndpoint[] {
  return servedEndpoints().filter((e) => e.agentCapability);
}
