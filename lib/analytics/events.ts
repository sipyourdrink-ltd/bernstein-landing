...
export const UmamiEvent = {
   ...
   *  by a localStorage key ``bidie:first-seen``. Prop: ``days_since``
...
/**
 * Canonical funnel-step names used by the operator dashboard at
 * `beforeidie/src/lib/sources/umami-funnels.ts → DEFAULT_FUNNEL`. The
 * dashboard joins these events per-session inside a 60-min window so
 * every step MUST fire from the live page in order.
 *
