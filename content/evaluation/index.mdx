---
title: "why bernstein"
description: "decision-support faq for engineers and compliance teams evaluating bernstein vs claude-flow, archon, vibe-kanban, claude-squad. honest who-it's-NOT-for sections, real numbers, no marketing voice."
date: "2026-05-12"
tags: ["multi-agent orchestration", "comparison", "claude code", "claude-flow", "archon", "vibe-kanban", "compliance", "decision support"]
---
...
<h2 id="why-not-claude-flow-archon">why bernstein over claude-flow, archon, vibe-kanban, or claude-squad</h2>

same shape - multi-agent cli orchestration in git worktrees - but bernstein has fewer stars than claude-flow, vibe-kanban, or archon. bernstein is not winning the star race in this category and is not going to. what it owns is a different column.

the auditability surface: hmac-chained audit log (every routing decision tamper-evident), signed agent cards (detached jws over jcs-canonicalized payloads with ed25519 keys), per-artefact lineage with customer-key signing (for dora / nis2 / eu ai act article 12 evidence), an air-gap deploy profile (`--profile airgap`), and python-library shape (the others are cli or desktop apps). that column is empty in every one of the bigger projects' readmes.
...
bernstein is the wrong pick when: you want the polished tui for parallel claude on your mac (claude-squad), the swarm framing with a large mcp tool catalog (claude-flow), a kanban board ui (vibe-kanban), or the workflow-yaml primitive with web ui and chat integration (archon). those are real, good products. they just don't carry the audit chain.

<h2 id="why-not-crewai-autogen">why bernstein over crewai, autogen, or langgraph (previous generation)</h2>

different shape entirely. their orchestrator is an llm driving python tool-calls; bernstein's orchestrator is plain python driving terminal coding agents (claude code, codex, aider, gemini cli) with one git worktree per agent. crewai / autogen / langgraph are good shapes for free-form research-style swarms where "let the model figure out who does what" is the whole point. they're the wrong shape if you want a deterministic plan that replays the same task graph twice. kept here for completeness because some teams still benchmark against them; the closer comparison is the cli-orchestrator section above.

<h2 id="why-not-claude-code">why bernstein over claude code alone</h2>

claude code can spawn sub-agents on its own; bernstein does the same thing across 48 different cli agents at once and verifies their output against your tests instead of trusting it - claude code included as one of the 48, wired through the same adapter interface as every other agent.
...
what works: the scheduler is deterministic, the merge queue lands what passes the gates, the audit log is hmac-signed and replays cleanly, there are 48 adapters in the registry, the install is one `pipx install bernstein`.
...
- non-coding tasks (research, writing, data analysis pipelines) - bernstein wraps cli coding agents specifically, not generic llm workflows. crewai, autogen, or langgraph are the right shape there
