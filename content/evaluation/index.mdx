...
claude code can spawn sub-agents on its own; bernstein does the same thing across 51 different cli agents at once and verifies their output against your tests instead of trusting it - claude code included as one of the 51, wired through the same adapter interface as every other agent.
...
what works: the scheduler is deterministic, the merge queue lands what passes the gates, the audit log is hmac-signed and replays cleanly, there are 51 adapters in the registry, the install is one `pipx install bernstein`.
